...
## Bug #16 — Coupon "WELCOME" crash trang Cart

**Độ khó**: ⭐⭐⭐⭐ (Medium-Hard)  
**Loại**: Linear multi-step, crash page  
**File**: `src/pages/Cart.jsx`

**Mô tả**: Tính năng Promo Code trên Cart page. Coupon "SAVE10" hoạt động bình thường (10% off). Nhưng coupon "WELCOME" crash vì API trả về `{ type: 'freeShipping', value: 'free' }` — value là **string** thay vì number. Code render discount gọi `coupon.value.toFixed(2)` → `TypeError: value.toFixed is not a function` vì `'free'.toFixed()` không tồn tại.
...
**Điểm mù**: Agent phải thử nhiều coupon code để phát hiện. SAVE10 hoạt động bình thường nên dễ bỏ qua.
